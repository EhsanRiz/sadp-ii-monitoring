...
interface InviteBody {
  ...
  /**
   * Where the invite email's "Accept invitation" link should send the user
   * after they're authenticated. Supabase normally uses Site URL; passing
   * this explicit override lets the frontend force `/set-password` so the
   * invitee lands on the welcome+password-setup screen.
   */
...
Deno.serve(async (req) => {
  ...
  const { email, full_name, phone = null, role, organization_id = null, redirect_to = null } = body;
